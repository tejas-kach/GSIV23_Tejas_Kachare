...
import { intialMovieList, updateMovieList } from "../slice/movie-slice.ts";
...
const Home = () => {
  const movie = useAppSelector((state) => state.moviesState.movies);
  ...
  const intialFetch = async (page: 1) => {
    ...
    dispatch(intialMovieList(movies));
    // setMovie([...movies]);
    setMovieData({
      ...movieData,
      page: page,
